...
public class LoggingMiddleware implements MiddlewareBase {
    ...
    public Flux<AgentEvent> onAgent(Agent agent,
        ...
        log.info("[mw:logging] -> onAgent  进入 agent={} sessionId={} userId={} 消息数={}",
                agent.getName(), ctx.getSessionId(), ctx.getUserId(), input.msgs().size());
        long t0 = System.nanoTime();
        return next.apply(input)
                .doOnComplete(() -> log.info("[mw:logging] <- onAgent  结束 耗时 {} ms",
                        (System.nanoTime() - t0) / 1_000_000))
                .doOnError(err -> log.warn("[mw:logging] <- onAgent  出错 err={}", err.toString()));
    }
    ...
    public Flux<AgentEvent> onReasoning(Agent agent,
        ...
        log.info("[mw:logging] -> onReasoning  进入 messages={} tools={} options={}",
    ...
    public Flux<AgentEvent> onModelCall(Agent agent,
        ...
        log.info("[mw:logging] -> onModelCall 进入 model={} messages={}",
                input.model().getClass().getSimpleName(), input.messages().size());
        return next.apply(input)
                .doOnComplete(() -> log.info("[mw:logging] <- onModelCall 完成"));
    }

    @Override
    public Mono<String> onSystemPrompt(Agent agent, RuntimeContext ctx, String currentPrompt) {
        log.info("[mw:logging] -> onSystemPrompt 进入 长度={}", currentPrompt.length());
